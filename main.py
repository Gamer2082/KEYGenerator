...
# variable
fronttext = f"Key generator " + "1.0.0"
bgcolor = '#413E44'
...
def ERREUR_LOGIN(erreur_message):
    ...
    label_error = Label(frame,
                        text="ERROR: function {generate_opt} is not call, please call the administrator if is an error",
...
label_title = Label(frame_main, text="KEY GENERATOR " + "1.0.0", background=bgcolor, font=("helvestica", 20),
                    bg=bgcolor, fg='white')
...
# button generator
pass_botton = Button(fird_frame, text="NEW", background=bgcolor, font=("helvestica", 20), fg='white',
                     command=Generateur_script)
...
file_menu.add_command(label="NEW", command=Generateur_script)
file_menu.add_command(label="RESET", command=reset_entry)
file_menu.add_command(label="QUIT", command=windows.quit)
